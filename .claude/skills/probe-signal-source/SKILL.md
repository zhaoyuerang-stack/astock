...
- `factory/autoresearch/generator.py::_SEEDS` —— 加 1-2 条种子。**置顶不再靠硬编码顺序**
  (ADR-034 后 `_steer_seed_order` 会按方向登记簿动态重排):要算力倾斜,在
  `knowledge/direction_registry.json` 加 `action=BOOST` 条目(带步骤 3 证据指针)。
...
### 步骤 8 — 结论回写方向登记簿(必做,含阴性结论;ADR-034)
probe 结果**必须**回写 `knowledge/direction_registry.json`(生成端 steering 的唯一策展输入),
否则死路会被反复重提、算力反复烧回去——这正是方向登记簿存在的原因:
- **阴性**(风格代理 / OOS 塌 / 太弱):加 `action=DEPRIORITIZE`(或确证死路 `SKIP`)条目,
  `evidence` 填本次 probe 报告路径/run_id,`revival_condition` 写清什么变了才值得重测,
  `expires` 给 180 天(到期自动复活重测)。
- **阳性**(正交且 OOS 不塌):加 `action=BOOST` 条目(同样带证据指针)。
- **证据门控**:无 evidence 的条目会被 `knowledge/directions.py` 忽略——别偷懒不填。
- 若本次是从 `knowledge/data_source_backlog.json` 领的任务,同时更新该条目状态(已 probe/结论指针)。
