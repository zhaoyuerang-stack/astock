...
> 目标:每天按这页跑通「数据 → 信号 → 纸面 → 监控 → 解读」。**系统不自动下单**,执行决策留给人。
...
# 1. 出当日信号(联网增量更新 + 质量校验 + 择时 + 持仓 + 调仓判断 → signals/YYYY-MM-DD.json)
python3 run_daily.py                 # 不想联网就加 --no-update,用现有数据
```
看输出 6 步:`[2]` 质量校验、`[3]` regime(🟢BULL/🔴BEAR)、`[5]` 调仓判断。
- **BEAR → 空仓**,闲置资金建议配 `511010 国债ETF`;**BULL → 全仓 illiq top-25**。
...
```
浏览器开 **http://localhost:3000**,九页看这些:
| 页面 | 每天看什么 |
|---|---|
...
```
定时任务(launchd):`scripts/ops/install_launchd_jobs.sh`(每日更新 / 每周维护 / 健康巡检)。
...
| 前端白屏 / chunk 404 | **别在 dev 跑 `npm run build`**(污染 `.next`);`rm -rf .next && npm run dev` + 硬刷新 ⌘⇧R |
...
## 能力边界(诚实)
跑通 = **研究决策闭环**(数据→信号→纸面→监控→解读),**刻意停在实盘执行之前**。
