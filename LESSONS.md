...
- **T+1 开盘执行的真实 gap（2026-06 初始记录 + 2026-07 v2 修正）**:
  * **初始记录** (`paper_replay` 2024-2025 重放): 真实盘 23.7%/-11.4%/Sh 1.29 vs 回测 45.1%/-10.9%/Sh 1.97, 差距 21.4pp。
  * **v2 统一定价口径修正 (2026-07-01)**: 21.4pp 的 gap 大部分来自后复权 vs raw 的估值口径差异, 不是纯执行时机差异。两边都用 raw_close 估值后, **纯 T+1 执行 gap = ΔSh -0.04, ΔAnn -0.9%**。涨跌停/停牌附加影响 ~0。
  * **修正结论**: 对 20 日调仓的 size60 top25 策略, T+1 开盘价执行的实质影响很小。之前"腰斩"的说法是测量错误。纸面回测与实盘之间的真正 gap 不在执行时机, 而在退市归零 (未量化)、微盘流动性冲击等尾部风险。
  * `scripts/research/paper_replay.py` (初始), `scratch/backtest_reality_v2.py` (v2 修正)。
...
## 大盘策略全部证伪——A 股 Top 800 无独立 alpha (2026-07-01)

### 重要限定：裸因子 vs 双阀风控

本结论适用于**裸大盘因子**（无风控层）。系统内存在一种例外：**大盘动量 + 双阀风控**（NAV MA40 门控 + 成交量熔断）在复合组合中可能有组合层贡献（见 "非相关多策略组合的回撤对消" 条目，2026-06-30），但该复合组合仅测试了 2023-2026 OOS（3 年），未经过完整 9-Gate/DSR/WF 验证，不能作为 standalone 在册证据。

### illiquidity-large-cap v1.0 年化 58.3% 确认为数据假象 (2026-07-01)

台账中 ann=58.3%, Sh=2.16 的数字来自 2010-2026 全区间，2010 段年化 102.9% 极异常（疑似复权/数据口径问题）。仅用 2018+ 数据重跑：Ann=-14.8%, DD=-85.8%, Sh=-0.42。8 年中 7 年亏损。**确认该策略没有真实 alpha。**

### 已测试的裸大盘方向（全部失败）

| 方向 | 信号类型 | 调仓周期 | 结果 |
|------|---------|:------:|:--:|
| 大盘 illiq (在 top-800 池用 Amihud) | 慢 | 20D | ann=58% 不可信, 已退役 |
| 大盘成长 (top-800 选大成交额) | 慢 | 20D | Sh=0.31, DD=-61% |
| 大盘质量 (top-800 选低波) | 慢 | 20D | Sh=0.09, 接近零 |
| 大盘动量对冲 | 慢 | 20D | Sh=-0.26, 全部退役 |
| 小盘+大盘等权混合 | 慢 | 20D | 任何混合比例都跑输纯小盘 |
| 小盘+大盘 regime 切换 | 慢 | 20D | 最优解=100%小盘 |
| 行业景气度过滤大盘 | 慢 | 20D | Sh=-0.09~0.33, 全部比纯大盘差 |
| 行业景气度加权大盘 | 慢 | 20D | Sh=-0.22, 更差 |
| 龙虎榜机构净买 (大盘池) | 快 | 1D | Sh=0.08, 日均仅8只有效股票 |
| 龙虎榜机构净买 (大盘池) | 快 | 3D/5D | Sh=-0.52/0.24 |
| 大盘+景气度 (大盘反应快) | 慢信号 | 1D/3D/5D | 越短越烂, 1D DD>99% |

**11 种方向、3 种信号源、5 种调仓周期——没有一种让大盘 Sharpe 超过 0.35。**

### 根因: 不是信号问题, 是市场结构问题

1. **大盘股被机构充分定价**——任何公开信号 (资金流、龙虎榜、行业景气度) 都已 price-in
2. **大盘池太小** (Top 800)——龙虎榜日均只覆盖 8 只, 选不够 25 只
3. **大盘股换手成本高**——短周期调仓的冲击成本吃掉一切
4. **A 股 alpha 只在微盘端**——limits-to-arbitrage 溢价, 机构被结构性挡在门外

### 教训

- **不要再用新信号救大盘。** 不是信号不够好, 是池子里没有 alpha。
- 行业景气度在大盘上的失败是同一个逻辑: 资金流入大盘股 = 公开信息 = 已定价。
- 唯一有效的大盘用法: 国债 ETF 511010 做防御腿 (跨资产, 不是 A 股内部)。
- **算力应该放到: 跨资产 (HK/债/黄金)、新的另类数据、或者接受单因子现状。**

## 行业景气度因子——Tushare 新数据源的第一个产出 (2026-07-01)

### 数据基础

新下载 3 个 tushare 数据集: pledge_stat (162万行), top10_holders (128万行), top_inst (84万行)。基于 moneyflow (1372万行) + stock_basic 行业映射, 构建行业资金流聚合。

### 因子公式

```python
景气度 = (flow_z + elg_z) / 2
  flow_z: 行业净流入占比的60日 rolling z-score
  elg_z:  行业超大单(机构)净占比的60日 rolling z-score
```

### 验证结果 (2021-2026)

**行业层面 (IC):**
- 截面 ICIR=0.04 (弱——同行业股票得分相同, 个股区分度低)
- 与 size60 截面相关 = **0.014** (完全正交)

**个股层面 (作为 size60 的否决器):**
- 排除 Z<-1 的行业: 全区间 Sh +0.10 (1.04→1.14), Ann +2.5pp, DD -3.7pp
- **6/6 年全部正改善**, 从未失效
- 2026 年效果最显著: Sh -0.93→-0.33 (+0.61)

**大盘层面:**
- 全部失败。景气度过滤/加权/排除过热均不能改善大盘策略。

### 严格测试

| 测试 | 结果 |
|------|:--:|
| OOS Train/Test (2021-2022→2023-2026) | ✅ ΔSh=+0.10 |
| 阈值敏感性 (plateau 检验) | ✅ [-2, -0.5] 区间全正 |
| Bootstrap 显著性 | ❌ p=0.118 (未达5%, 88%置信) |

### 结论与定位

- **方向确定、量级不够。** 6/6年正改善、OOS通过、参数稳健——信号真实但样本不足以统计证实。
- **定位: SHADOW 观察。** 不作为 standalone 因子, 挂在 size60 上做可选行业过滤器。
- **不要用它做正选择 (买景气行业)。** 景气度高 = 资金已流入 = 已 price-in。只有做负选择 (避开资金逃逸行业) 才有效。
- **不要用它改善大盘。** 大盘股定价效率高, 行业资金流已是公开信息, 无超额。记入大盘证伪记录。
- 因子代码: `factors/industry_prosperity.py`, 测试脚本: `scratch/backtest_reality_v2.py`

## 回测执行现实核对 — 验证结论 (2026-07-01)

### T+1 执行 gap 修正
v2 统一定价口径实验: 两边都用 raw_close 估值后, 纯 T+1 开盘价执行 gap = ΔSh -0.04, ΔAnn -0.9%。之前"年化腰斩 21.4pp"是后复权 vs raw 口径混杂。涨跌停/停牌影响 ~0 (20日调仓微盘策略天然避开)。**回测引擎离现实的距离比之前担心的近得多。**

### 退市归零敏感性分析
蒙特卡洛模拟 (200次/情景): 即使 2% 年退市率 (远高于实际 ~0.5%), 年化拖累仅 ~2pp。退市归零本身对净值影响有限。**真正危险的是退市前连续跌停无法卖出**, 不是退市本身。该尾部风险需要在 paper_trade 建模。

### illiquidity-large-cap v1.0 确认数据假象
台账 ann=58.3%, Sh=2.16 来自 2010-2026。仅用 2018+ 重跑: Ann=-14.8%, DD=-85.8%, Sh=-0.42。8年7亏。确认无真实 alpha, 已退役状态正确。

## 正交增强件全景 (2026-07-01 最终)
...
| 增强件 | 效果 | 原理 |
|--------|:--:|------|
| MA16 PureTrend | Sh +0.12, DD -14pp | 趋势向下空仓 |
| Band 连续缩放 | Calmar +13% | dist越大仓位越大 |
| 国债 ETF 511010 | Sh +0.68 | 跨资产, 熊市不躺现金 |
| **波动率加速度** | Sh +0.05, DD +2pp | 波动率飙升=regime切换, 降仓躲过 |

**截面维 (1):**
| 增强件 | ΔSh | 截面相关 | 逐年 | OOS |
|--------|:---:|:------:|:---:|:--:|
| **pledge 排除高质押30%** | **+0.27** | -0.03 | 5/6年正 | +0.19 |

**待观察 (1):**
| 增强件 | ΔSh | 状态 |
|--------|:---:|:--:|
| 行业景气度排除 Z<-1 | +0.10 | 6/6年正但 bootstrap 不显著(p=0.118) |

### 证伪 (6):
block折价大宗(-0.13)、inst龙虎榜(-0.62,PIT修正后)、holders机构减持(-0.07)、loser_veto_reversal(-0.06)、alpha101否决器(+0.03)、ST排除(负效果)。

### 突破20日高 — 修正: 含未来函数, 修正后改善有限 (2026-07-02)

**初版存在未来函数**: `signal.iloc[idx]=1.0` 使当天收盘判断的突破立即生效, 偷看了当天收盘价。修正为 `idx+1` 起始 + 通过 engine T+1 timing 后, OOS Test:
- 微盘: 突破 Sh=1.50 vs raw Sh=1.15 (+0.35), MA16 Sh=1.40 — 仍有改善但缩水
- 800-2000(asc): 突破 Sh=0.92 vs raw Sh=0.59 (+0.33) — 有改善
- 501-800(desc): 突破 Sh=0.33 vs raw Sh=0.31 — **几乎无改善**
- 501-800(asc): 突破 Sh=0.92 vs raw Sh=0.93 — 无改善, MA16 更好

**"跨宇宙通用"不成立。** 大盘端(top-50~501-800 desc)无改善或负改善。中盘改善仅在成交额从小到大排序时有效。微盘改善仍在但量级从+1.24缩到+0.35。用法: 仅作候选时序过滤器, 需 T+1 可交易口径验证。

### 增强件矩阵 — 里程碑 (2026-07-01)

**size60+pledge+MA16 首次通过 Gate 5（真实成本回测），DD=-17.5% 首次压到 20% 入册门槛以下。**

完整 9-Gate: 8/10 通过。仅 G4(DSR_p=0.23) 和 G6(容量衰减) 失败。
- G5: Ann=21.2%, DD=-17.5%, Sh=1.23, Cal=1.21
- G3 中性化保留率 153% — 中性化反升, 确认真特质信号
- G7 WF_Sh=1.28, 但 Bear=-4.48 显示极端 regime 依赖 (微盘固有特征)
- G4 失败原因: DSR 门槛为 standalone 设计, 组合增强件的 n_trials 累积使其无法通过
- G6 失败原因: 微盘容量物理上限 ~2千万, 放大后 Sharpe 衰减

### 波动率加速度 — 二阶优于一阶 (2026-07-01)

借鉴 Barroso & Santa-Clara (2015) "Momentum has its moments" 的波动率缩放思路:
- **一阶 (波动率水平)**: 高波降仓、低波加仓 → 对微盘策略无效 (Sh不变, DD更差)
- **二阶 (波动率加速度)**: 波动率在飙升=危险来临降仓, 在回落=风暴过去加仓 → **有效**

叠加在 MA16 之上: Sh 1.32→1.37, DD -18.3%→-16.4%。与 MA16 互补: MA16 看方向(价格趋势), 波动率加速度看烈度(风险变化)。论文解决的是动量策略的短促崩溃, 而我们适配为微盘策略的 regime 切换预警。

## 复合策略(Composite)数据对齐与多重平移时钟错配漏洞 (2026-07-05)

### 1. 行对齐重采样缺失 (NaN 泄露)
- **教训**：在将字典或 Series 转化成多标的持仓矩阵时，如果只做列对齐 `.reindex(columns=close.columns)`，而**漏掉了行重采样 `.reindex(index=close.index)`**，DataFrame 就仅包含调仓日那几十行。在 DataFrame 与全时序 Series 相乘时，由于对齐机制，未调仓日（占95%以上）在并集后全被填为了 `NaN`。这不仅会导致 Gate 0 数据审计发出严重警告，更会导致回测引擎由于 target_weight 大面积失效而将年化收益从 18.7% 级严重稀释至 4.2%。
- **规范**：**任何策略权重的 Runner 生成最终 weights 时，必须强制在行和列上做双重对齐**：
  `w_base = df.reindex(index=close.index, columns=close.columns).ffill().fillna(0.0)`

### 2. 信号自带时滞叠加 (Double-Shift)
- **教训**：有些策略底层（如 `small_cap_timing`）出于实盘部署考虑，已经在算法内部执行了 `.shift(1)` 时延。如果将该 Leg 引入 Composite 组合，且晋级回测引擎默认又对 Signal 执行了一次 `T_PLUS_1_CLOSE` 物理延迟，就会形成 **T+2 双重平移**，在遇到剧烈风格踩踏时，择时阀门的反应会被迫滞后 2 天甚至 3 天，导致择时防守形同虚设，净值受到重创。
- **规范**：**对于在内部已嵌入 shift 的 Leg 择时，拼装组合时必须使用 `timing.shift(-1).ffill().fillna(True)` 向前移动，以抵消引擎的 T+1 平移**，确保在组合层仅承受唯一的单日执行延迟。

### 3. Bool 乘积导致 Object 隐式类型升级
- **教训**：布尔型 Series 在与 float DataFrame 相乘时，Pandas 会自动将其强制转换为 `object`（混合类型），这会导致 Gate 0 的 `np.isinf()` 计算抛出 `TypeError: ufunc 'isinf' not supported for the input types` 导致审计流水线整体崩溃。
- **规范**：**在矩阵乘法前，必须显式对 Bool 系列执行强制类型转换**：
  `timing_daily = timing.reindex(close.index).ffill().fillna(0.0).astype(float)`
