...
- **多策略组合均线择时参数的过拟合实证 (2026-06)**：运行 `audit_portfolio_regime.py` 审计表明，组合动态择时（在 A 股/ETF 之间轮动）对移动平均线窗口长度极度敏感且面临严重的过拟合风险。在 9 种 MA 参数 variants 测试下：① **PBO (过拟合概率)** 极高（方案一 71.0%，方案二 35.0%）；② **DSR (Deflated Sharpe)** p-value 达 **0.1355**，未能通过 0.05 的多重测试显著性门槛（原因在于组合收益的超额峰度高达 15.62，导致标准误差偏大）。这用实证数据证明：多策略层的择时参数必须保持刚性（如固定 16 日），切忌在回测中微调参数，否则样本外极易因过拟合而造成踏空与调仓磨损。
