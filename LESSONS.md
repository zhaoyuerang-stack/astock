...
- **北向个股资金源当前 blocked(2026-06)**:`ak.stock_hsgt_stock_statistics_em` / Eastmoney `RPT_MUTUAL_STOCK_NORTHSTA` 返回 `9701 服务器繁忙` 或 `None result`;代码已接 `NorthboundFetcher`,但本机网络/东财状态恢复前无法落完整 `northbound_all.parquet`。北向未验证,不能用空数据得结论;后续先修 eastmoney 直连/代理规则再补跑。
