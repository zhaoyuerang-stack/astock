...
- 真实口径基线：`data_lake` 2018-2026 年化约 31.9%，最大回撤约 -11.9%
...
- `evolve.py`：旧研究阶段沉淀的因子、回测和指标工具。
- `scripts/data/`：数据构建、拉取和增量更新脚本。
- `scripts/repair/`：数据修复和重校验脚本。
- `scripts/research/`：旧研究、搜索、模拟和成本敏感性脚本。
- `scripts/diagnostics/`：诊断和问题排查脚本。
- `data_full/`：旧口径价量缓存，仅用于历史对比。
...
优先级最高的是在真实 `data_lake` 口径上继续迭代 v2 策略，而不是回到 `data_full` 旧口径追求达标。
...
- `results/FINAL_config.json`：最终策略参数和样本内/样本外指标。
- `results/FINAL_strategy.png`：最终策略净值和回撤图。
- `results/best_config.json`：进化搜索历史最优配置。
- `results/evolution_history.csv`：进化过程记录。
