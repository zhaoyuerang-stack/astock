"""Sweep rebalancing frequencies for untimed active strategies to isolate core factor turnover.
"""
import os
import sys
import warnings
...
def run_sweep(strategy_name, factor_builder, start_date="2018-01-01"):
    print(f"\nRunning untimed frequency sweep for: {strategy_name}")
    ...
    for freq in frequencies:
        ...
        # Config with standard costs (no leverage here for simplicity)
        cfg = BacktestConfig(
            start=start_date,
        ...
        signal = Signal(
            ...
            version=f"sweep_f{freq}_untimed",
        )
...
def main():
    print("=" * 80)
    print("  UNTIMED REBALANCING FREQUENCY SWEEP (CORE FACTOR TURNOVER)")
    print("=" * 80)
