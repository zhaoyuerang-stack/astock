"""Research script to backtest and optimize Chips Distribution Factor (CDF).
"""
import os
import sys
import warnings
...
def load_shares_outstanding(trade_dates, data_lake_path=Path("data_lake")):
    """Calculates shares outstanding from fundamental data and aligns to trade calendar."""
    fund = pd.read_parquet(data_lake_path / "fundamental_batch.parquet")
    fund["avail_date"] = pd.to_datetime(fund["avail_date"])
    
    # Calculate shares outstanding (Net Profit / EPS)
...
def main():
    print("=" * 110)
    print("  CHIPS DISTRIBUTION FACTOR (CDF) OPTIMIZATION AND BACKTEST  ")
    print("=" * 110)
    ...
    close = panels["close"]
    volume = panels["amount"] * 0.0  # Empty volume for engine
    amount = panels["amount"]
    raw_close = panels["raw_close"]
    
    # Real volume for turnover calculation
    ...
    # 2. Compute shares outstanding and daily turnover rate
    print("\n[2/4] Computing shares outstanding and daily turnover rate...", flush=True)
    shares = load_shares_outstanding(clean_dates).reindex(columns=close.columns).ffill()
    
    # Turnover = volume(lots) * 100 / shares
    turnover = (volume_raw * 100.0 / shares.replace(0, np.nan)).fillna(0.0)
    # Clip turnover to logical bounds [0, 0.50] to avoid extreme split/data anomalies
    turnover = turnover.clip(lower=0.0, upper=0.50)

    # 3. Compute cost distribution average and standard deviation recursively
    print("\n[3/4] Running recursive cost decay filters...", flush=True)
    
    avg_cost = pd.DataFrame(np.nan, index=clean_dates, columns=close.columns)
    cost_var = pd.DataFrame(np.nan, index=clean_dates, columns=close.columns)
    
    # Fast numpy-level loop for recursive decay
    ...
    # Concentration = Average Cost / Cost Std
    concentration = avg_cost / (cost_std + 1e-8)
    
    # Profit Ratio = norm.cdf((close - avg_cost) / cost_std)
    z_score = (close - avg_cost) / (cost_std + 1e-8)
    # Vectorized norm.cdf
    profit_ratio = pd.DataFrame(norm.cdf(z_score), index=clean_dates, columns=close.columns)
    
    # Chips Factor = Concentration * (Profit Ratio - 0.50)
    cf = concentration * (profit_ratio - 0.50)

    # 4. Backtest in Top 800 Universe
    ...
    # Factor variants:
    factors = {
        "concentration": concentration.where(univ_800),  # Pure Concentration (Main force lockup)
        "profit_ratio": profit_ratio.where(univ_800),    # Pure Profit Ratio
        "chips_factor": cf.where(univ_800)               # Combined Chips Factor
    }
    ...
    print("\n" + "=" * 95)
    print(f"  CHIPS STRATEGY RESULTS COMPARISON (2012-01-01 to 2026-06-09)")
    print("=" * 95)
    print(f"{'Strategy Variant':<20} | {'Long AnnRet':<12} | {'Hedged AnnRet':<13} | {'Hedged Sharpe':<13} | {'Hedged MaxDD':<12}")
    print("-" * 95)
    for name, ml, mh in results:
        print(f"{name:<20} | {ml['annual']:>12.2%} | {mh['annual']:>13.2%} | {mh['sharpe']:>13.2f} | {mh['maxdd']:>12.2%}")
    print("=" * 95)
