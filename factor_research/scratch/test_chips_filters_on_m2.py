"""Research script to test chips distribution indicators as defensive risk filters on M2.
"""
import os
import sys
import warnings
...
def main():
    print("=" * 110)
    print("  EVALUATING CHIPS FILTERS ON LARGE-CAP STRATEGY (M2)  ")
    print("=" * 110)
    ...
    # 1. Build Base M2 Factor (w_cpv_max = 0.25)
    print("\n[1/3] Computing base M2 factors...", flush=True)
    ...
    # Filter 2: Exclude stocks where Profit Ratio < 25% OR (Concentration > 5.0 and Profit Ratio < 50%)
    filter_no_breakdown = ~( (profit_ratio < 0.25) | ((concentration > 5.0) & (profit_ratio < 0.50)) )
    factor_f2 = base_factor.where(filter_no_breakdown)

    # 3. Run Backtests (2012-2026 OOS-aligned)
    print("\n[2/3] Running backtests for timed & hedged M2...", flush=True)
    eval_start = "2012-01-01"
    
    # Timed/Hedged run function helper
    def run_m2_timed(factor):
        ...
        return r_timed.loc[eval_start:]

    print("\n[3/3] Evaluating returns...", flush=True)
    ret_base = run_m2_timed(base_factor)
    ret_f1 = run_m2_timed(factor_f1)
    ret_f2 = run_m2_timed(factor_f2)
    
    m_base = get_metrics(ret_base)
    m_f1 = get_metrics(ret_f1)
    m_f2 = get_metrics(ret_f2)
    
    print("\n" + "=" * 90)
    print("  M2 PERFORMANCE WITH CHIPS FILTERS (2012-01-01 to 2026-06-09)")
    print("=" * 90)
    ...
    print(f"M2 with Filter 2 (No Breakdown)  | AnnRet: {m_f2['annual']:>7.2%} | Sharpe: {m_f2['sharpe']:>5.2f} | MaxDD: {m_f2['maxdd']:>7.2%} | Calmar: {m_f2['calmar']:.2f}")
    print("=" * 90)
