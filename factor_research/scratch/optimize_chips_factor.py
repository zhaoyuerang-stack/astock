"""Research script to search for reversed alpha in the chips distribution factor.
"""
import os
import sys
import warnings
...
def main():
    print("=" * 115)
    print("  REVERSED CHIPS FACTOR ALPHA OPTIMIZATION  ")
    print("=" * 115)
    ...
    # Define reversed/oversold variants
    factors = {
        "rev_concentration": -concentration.where(univ_800),  # Dispersed chips (buying breaks)
        "rev_profit_ratio": -profit_ratio.where(univ_800),    # Reversal / Capitulation bottoms
        "oversold_concentr": (concentration * (0.50 - profit_ratio)).where(univ_800), # Consolidated panic bottoms
        "rev_chips_factor": -cf.where(univ_800)               # Contrarian chips breakout
    }
