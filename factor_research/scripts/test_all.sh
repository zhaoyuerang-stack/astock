...
# F841 B006 B007 UP031 UP042 为存量基线(2026-07-21 评审在案,
# 分批清零中:B008/B023/B905 已 2026-07-21 清零并解禁,余类清零前豁免);
# 结构性豁免见 pyproject per-file-ignores(归档已 exclude)。
if ! python3 -m ruff --version >/dev/null 2>&1; then
  echo "❌ 当前 python3 无 ruff 模块,请用项目解释器运行或安装 ruff(≥0.15)"
  exit 1
fi
python3 -m ruff check --select F,B,I,UP --ignore F841,B006,B007,UP031,UP042 .
