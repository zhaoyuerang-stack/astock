"""illiquidity 模拟盘(真实盘成交逻辑):T+1 按 FILL_PRICE_MODE 成交 + pending + 停牌/涨跌停。

真实盘口径(与 portfolio.paper_engine 同源,展示层禁止硬编码「开盘/收盘」):
  · T 日盘后出信号 → 记 pending → 次日按 FILL_PRICE_MODE 成交(默认 **close** 收盘价)
  · 环境变量 PAPER_FILL_MODE 可覆盖 open|close|ohlc_mid|vwap_4
  · 停牌不可买卖;一字涨跌停约束仍按**次日开盘价**判定(即使成交价是 close)
  · 等权 1/top_n,A股 100 股整数倍;成交/估值全用不复权价(daily_raw)
  · 本金 100 万,1.0x;成本走 CostModelConfig
执行引擎: portfolio/paper_engine.py(与 web 操作卡 services/read/paper.py 共享)
状态: paper/account.json + trades.csv + nav.csv
用法(cwd=factor_research): python3 -m scripts.ops.paper_trade [--preview]
"""
import argparse
import json
import os
...
from portfolio.paper_engine import (  # noqa: E402
    ...
    FILL_PRICE_MODE, fill_mode_label, fill_mode_zh, fill_mode_disclaimer,
...
def render_card(date, signal, decay, acc, nav, pos_value, detail, trades, blocked, names, exec_from):
    ...
    fill_lbl = fill_mode_label()  # 跟 paper_engine.FILL_PRICE_MODE,禁写死「开盘」
    fill_zh = fill_mode_zh()
    lines = [
        title,
        "",
        f"> 自动生成 {datetime.now(CHINA_TZ):%Y-%m-%d %H:%M} CST | illiquidity {strategy_ver} | 本金 {fmt(acc['init_capital'])} | "
        f"杠杆 {LEVERAGE}x | 真实盘 {fill_zh}(mode={FILL_PRICE_MODE})",
        stale_note,
        f"## 📋 今日成交({fill_lbl})" + (f"(执行 {exec_from} 信号)" if exec_from else ""),
        "",
    ]
    if not exec_from:
        lines += ["今日无待执行订单(模拟盘首日或上一信号为空仓)。", ""]
    elif not trades and not blocked:
        lines += [f"目标与持仓一致,今日无买卖({fill_lbl})。", ""]
    else:
        if sells:
            lines += [f"**卖出 {len(sells)} 只({fill_lbl}):**", "",
                      f"| 代码 | 名称 | 股数 | {fill_lbl} | 金额 |", "|--|--|--|--|--|"]
            lines += [f"| {t[1]} | {t[2]} | {t[4]} | {t[5]} | {fmt(t[6])} |" for t in sells]
            lines += [""]
        if buys:
            tot = sum(t[6] for t in buys)
            lines += [f"**买入 {len(buys)} 只({fill_lbl},合计 ≈ {fmt(tot)}):**", "",
                      f"| 代码 | 名称 | 股数 | {fill_lbl} | 金额 |", "|--|--|--|--|--|"]
    ...
    if signal["in_market"] and target:
        ...
        lines += [f"**空仓观望**,明日不建仓(若有持仓则次日按 {fill_lbl} 清仓)。", ""]
    ...
    if pend_bond.get("enabled"):
        ...
        if est:
            ...
            lines += [f"**🔄 债券轮动(BEAR)**:次日闲置资金买入 {pend_bond.get('code','511010')} "
                      ...
                      f"(参考价=今收,实际 {fill_lbl},费率 {ETF_BUY_COST:.2%})", ""]
        ...
        if defensive_bond_authorized(pend_bond):
            lines += [f"**🔄 债券轮动(BULL)**:次日按 {fill_lbl} 卖出全部 {acc['bond']['code']} "
    ...
    lines += [
        "## 📖 卖出规则(系统自动执行)", "",
        f"- 择时转空(小盘指数跌破 MA16)→ 次日按 {fill_lbl} **全部清仓**",
        f"- 调仓日(每 20 交易日)→ 次日按 {fill_lbl} 卖掉**掉出 top25** 的、买入新进的",
        "- 无单只止盈止损(截面策略,靠分散+调仓控制风险)",
        "", "## ⚠️ 口径声明", "",
        f"- **真实盘成交**:{fill_mode_disclaimer()} "
        f"(FILL_PRICE_MODE=`{FILL_PRICE_MODE}`,可用 PAPER_FILL_MODE 覆盖)",
        "- **停牌/涨跌停**:停牌不可买卖;一字涨跌停约束看次日**开盘价**(即使成交价是收盘价)",
        ...
        "- 回测口径另算——回测归回测,本卡是模拟盘真实买卖逻辑",
    ]
    return "\n".join(lines) + "\n"


def build_preview(names):
    """假设择时转多:展示「次日按 FILL_PRICE_MODE 建仓清单」(不碰正式账户)。"""
    from strategies.small_cap import load_price_panels
    from factors.utils import safe_zscore, mad_clip
    from factors.small_cap import small_cap_timing
    fill_lbl = fill_mode_label()
    ...
    warn = (
        f"> 🔮 **建仓预览** —— 假如择时转多,系统会在**次日按 {fill_lbl}** 建仓。\n"
        f"> ⛔ **当前实际择时 🔴 空仓({sig['timing_dist']:+.2%} vs MA16),切勿照此买入!**\n"
        f"> 参考价=信号日({date})收盘,实际按 {fill_mode_zh()}(mode=`{FILL_PRICE_MODE}`)。\n\n"
    )
    ...
    print(f"=== 建仓预览 {date}(假设转多,次日 {fill_lbl})===")
...
def main():
    ...
    ap.add_argument(
        "--preview",
        action="store_true",
        help=f"生成次日按 {fill_mode_label()} 建仓预览,不碰正式账户",
    )
    args = ap.parse_args()
    if args.preview:
    ...
    if acc["last_date"] != date:
        # 1. 结算上次 pending:用 date(=上个信号的次日)按 FILL_PRICE_MODE 成交
        ...
        # 2. 记录新 pending:本信号 → 下个交易日按 FILL_PRICE_MODE 执行
    ...
    print(f"=== illiquidity v1.0 模拟盘({fill_mode_zh()}, mode={FILL_PRICE_MODE}) {date} ===")
    print(f"  今日成交({fill_lbl}): 买{len([t for t in trades if t[3]=='BUY'])} 卖{len([t for t in trades if t[3]=='SELL'])} "
          f"受阻{len(blocked)}" + (f"(执行{exec_from}信号)" if exec_from else "(无待执行)"))
    nxt = (acc.get("pending") or {}).get("target") or []
    print(f"  明日计划: {'建仓/持有 '+str(len(nxt))+' 只' if (signal['in_market'] and nxt) else '空仓观望'}"
          f" → 次日 {fill_lbl}")
