...
"""Fine-grained threshold sweep for HMM macro exit.

Threshold: 0.05 ~ 0.60, step 0.05 × floor: [0.0, 0.2, 0.3, 0.5]
With yearly breakdown for the best combo.
...
def main():
    ...
    thresholds = [round(x, 2) for x in np.arange(0.05, 0.65, 0.05)]
    floors = [0.0, 0.2, 0.3, 0.5]
    configs = [(t, f) for t in thresholds for f in floors]

    print(f"\nThreshold sweep: {len(configs)} combinations...", flush=True)
    for i, (th, fl) in enumerate(configs):
        exposure = (stress_prob.reindex(close.index).fillna(0.0) <= th).astype(float) * (1.0 - fl) + fl
        timing = smallcap_timing.reindex(close.index).fillna(0.0) * exposure
        ret, detail = backtest_weights(close, scheduled, timing, cfg)

        label = f"th{th:.2f}_floor{fl:.1f}"
        ...
        stress_days = int((stress_prob.reindex(close.index).fillna(0.0) > th).sum())
        if (i + 1) % 10 == 0 or i == 0:
            print(f"  [{i+1}/{len(configs)}] {label:<18} "
    ...
    result.to_csv(OUT_DIR / "hmm_macro_threshold_sweep.csv", index=False)
    ...
    best_th, best_fl = best["threshold"], best["floor"]
    print(f"\n=== Yearly breakdown for best: th={best_th:.2f}, floor={best_fl:.1f} ===", flush=True)

    exposure = (stress_prob.reindex(close.index).fillna(0.0) <= best_th).astype(float) * (1.0 - best_fl) + best_fl
    timing = smallcap_timing.reindex(close.index).fillna(0.0) * exposure
    ...
    for year in sorted(set(baseline_ret.index.year)):
        ...
        stress_days = int((sp > best_th).sum())
    ...
    for th in thresholds:
        sub = result[(result["threshold"] == th) & (result["floor"] == 0.0)]
        if sub.empty:
            continue
        row = sub.iloc[0]
        # Compute yearly returns for this threshold
        exposure = (stress_prob.reindex(close.index).fillna(0.0) <= th).astype(float)
        timing = smallcap_timing.reindex(close.index).fillna(0.0) * exposure
        ret_y, _ = backtest_weights(close, scheduled, timing, cfg)
        yr = {}
        for y in bull_years:
            b = baseline_ret[baseline_ret.index.year == y]
            r = ret_y[ret_y.index.year == y]
            yr[y] = (1 + r.fillna(0)).prod() - 1 - ((1 + b.fillna(0)).prod() - 1)
        print(f"{th:>10.2f} {yr[2015]:>+7.1%} {yr[2021]:>+7.1%} {yr[2025]:>+7.1%} {row['sharpe_2018']:>8.2f}", flush=True)

    print(f"\nWrote: {OUT_DIR / 'hmm_macro_threshold_sweep.csv'}", flush=True)
