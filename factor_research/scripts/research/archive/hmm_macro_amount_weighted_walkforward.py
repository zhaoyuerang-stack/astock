...
"""Walk-forward with amount-weighted market return (alternative to equal-weight).

Hypothesis: amount-weighted mkt_ret is more accurate because it reflects
actual capital flows (not just number of stocks). If tw=3 still wins,
the trend filter is robust to data weighting choice.

Usage:
  cd /Users/kiki/astcok/factor_research && python3 scripts/research/hmm_macro_amount_weighted_walkforward.py
"""
import json
import os
import sys
...
def mkt_ret_amount_weighted(close, amount):
    """Amount-weighted market return (alternative to equal-weight).

    Only uses stocks with positive amount and valid return.
    """
    ret = close.pct_change(fill_method=None)
    has_trade = amount > 0
    valid = ret.notna() & has_trade
    weighted = (ret.fillna(0.0) * amount * valid).sum(axis=1)
    total_amount = (amount * valid).sum(axis=1)
    return weighted / total_amount.replace(0, np.nan)
...
def main():
    ...
    print("Building stress signal (unchanged)...", flush=True)
    stress_prob = build_stress_signal(features, lookback=250, retrain_days=60)

    print("Computing amount-weighted market return (NEW)...", flush=True)
    mkt_ret_aw = mkt_ret_amount_weighted(close, amount)
    mkt_ret_ew = close.pct_change(fill_method=None).mean(axis=1)

    print(f"  Equal-weight:  range=[{mkt_ret_ew.min():.3f}, {mkt_ret_ew.max():.3f}]  mean={mkt_ret_ew.mean():.4f}")
    print(f"  Amount-weight: range=[{mkt_ret_aw.min():.3f}, {mkt_ret_aw.max():.3f}]  mean={mkt_ret_aw.mean():.4f}")
    corr = mkt_ret_ew.corr(mkt_ret_aw)
    print(f"  Correlation: {corr:.3f}")

    # Use amount-weighted for walk-forward
    windows = [2, 3, 5, 7, 10, 15, 20]
    thresholds = [0.05, 0.10, 0.20]
    floors = [0.0, 0.2]
    combos = [(t, f, w) for t in thresholds for f in floors for w in windows]
    print(f"\nCombo grid: {len(combos)} combinations (using amount-weighted mkt_ret)", flush=True)

    print("Pre-computing combo returns...", flush=True)
    combo_rets = {}
    for i, (th, fl, w) in enumerate(combos):
        ret = run_combo(close, scheduled, smallcap_timing, cfg, stress_prob, mkt_ret_aw, th, fl, w)
    ...
    print(f"\nWalk-forward: {len(wf_years)} years (amount-weighted)...", flush=True)
    wf_returns = []
    baseline_annual = []
    fixed_tw3_aw = []  # amount-weighted tw=3
    fixed_tw3_ew = []  # equal-weight tw=3 (for comparison)
    param_history = []

    fixed_tw3_aw_ret = combo_rets[(0.05, 0.0, 3)]

    # Also compute equal-weight tw=3 for direct comparison
    fixed_tw3_ew_ret = run_combo(close, scheduled, smallcap_timing, cfg,
                                   stress_prob, mkt_ret_ew, 0.05, 0.0, 3)
    ...
    for y in wf_years:
        ...
        fixed_aw_oos = fixed_tw3_aw_ret[fixed_tw3_aw_ret.index.year == y]
        fixed_ew_oos = fixed_tw3_ew_ret[fixed_tw3_ew_ret.index.year == y]
        base_oos = baseline_ret[baseline_ret.index.year == y]

        wf_ann = (1 + oos_r.fillna(0)).prod() - 1
        fixed_aw_ann = (1 + fixed_aw_oos.fillna(0)).prod() - 1
        fixed_ew_ann = (1 + fixed_ew_oos.fillna(0)).prod() - 1
        base_ann = (1 + base_oos.fillna(0)).prod() - 1

        wf_returns.append(wf_ann)
        fixed_tw3_aw.append(fixed_aw_ann)
        fixed_tw3_ew.append(fixed_ew_ann)
        ...
        param_history.append({
            ...
            "oos_ret": wf_ann,
            "fixed_aw_ret": fixed_aw_ann,
            "fixed_ew_ret": fixed_ew_ann,
            "base_ret": base_ann,
        })
        ...
        print(f"  {y}: IS best=(th={th:.2f}, fl={fl:.1f}, tw={w:>2d}) "
              ...
              f"WF={wf_ann:+.1%} AW-tw3={fixed_aw_ann:+.1%} EW-tw3={fixed_ew_ann:+.1%} Base={base_ann:+.1%}", flush=True)

    # Summary
    wf_series = pd.Series(wf_returns, index=wf_years)
    aw_series = pd.Series(fixed_tw3_aw, index=wf_years)
    ew_series = pd.Series(fixed_tw3_ew, index=wf_years)
    base_series = pd.Series(baseline_annual, index=wf_years)

    print("\n=== Walk-Forward Summary (2015-2026) ===", flush=True)
    print(f"{'Metric':<20} {'WF-AW':>14} {'Fixed-AW':>14} {'Fixed-EW':>14} {'Baseline':>14}", flush=True)
    print(f"{'Annualized':<20} {wf_series.mean():>+13.1%} {aw_series.mean():>+13.1%} {ew_series.mean():>+13.1%} {base_series.mean():>+13.1%}", flush=True)
    print(f"{'Sharpe':<20} {sharpe_of(wf_series):>13.2f} {sharpe_of(aw_series):>13.2f} {sharpe_of(ew_series):>13.2f} {sharpe_of(base_series):>13.2f}", flush=True)
    print(f"{'Min Yearly':<20} {wf_series.min():>+13.1%} {aw_series.min():>+13.1%} {ew_series.min():>+13.1%} {base_series.min():>+13.1%}", flush=True)
    print(f"{'Win vs Base':<20} {(wf_series > base_series).mean():>13.0%} {(aw_series > base_series).mean():>13.0%} {(ew_series > base_series).mean():>13.0%}", flush=True)
    ...
    # Comparison: AW vs EW
    print("\n=== AW vs EW Comparison (tw=3, th=0.05, fl=0.0) ===", flush=True)
    print(f"{'Year':<6} {'Base':>9} {'AW':>9} {'EW':>9} {'AW-Base':>9} {'EW-Base':>9} {'AW-EW':>9}", flush=True)
    for y in wf_years:
        b = baseline_ret[baseline_ret.index.year == y]
        a = fixed_tw3_aw_ret[fixed_tw3_aw_ret.index.year == y]
        e = fixed_tw3_ew_ret[fixed_tw3_ew_ret.index.year == y]
        b_ann = (1 + b.fillna(0)).prod() - 1
        a_ann = (1 + a.fillna(0)).prod() - 1
        e_ann = (1 + e.fillna(0)).prod() - 1
        print(f"{y:<6} {b_ann:>+8.1%} {a_ann:>+8.1%} {e_ann:>+8.1%} "
              f"{a_ann-b_ann:>+8.1%} {e_ann-b_ann:>+8.1%} {a_ann-e_ann:>+8.1%}", flush=True)

    df = pd.DataFrame(param_history)
    df.to_csv(OUT_DIR / "hmm_macro_amount_weighted_walkforward.csv", index=False)
    print(f"\nWrote: {OUT_DIR / 'hmm_macro_amount_weighted_walkforward.csv'}", flush=True)
