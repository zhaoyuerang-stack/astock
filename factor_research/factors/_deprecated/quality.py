"""DEPRECATED (R-ARCH-005) — 财务质量类因子（基于季报数据）。

历史沉淀:标量/单票 helper,非 date×code 面板,从未接入 catalog/DSL/ALLOWED_FACTORS。
正式质量信号见 factors.fundamental (roe/gross_margin/cfo_quality)。
本模块不作实现依据;勿 import 进新研究路径。
"""
import pandas as pd
import numpy as np
