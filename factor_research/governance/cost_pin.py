"""R-COST-001 成本钉死唯一权威：CostModel 三费率快照/哈希(hash-pin)逻辑。
...
本模块是唯一权威(canonical)——lake.version_returns 等下层模块依赖本模块取
cost_hash 口径(此前反向 import scripts/ci/check_cost_model_pin.py,已迁出)。
