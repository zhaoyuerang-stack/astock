...
- [ ] **【数据层·高优先】退市归零损失建模(清单 G)** — canonical 回测对持仓中退市股**静默消失**,不按退市整理期价格清算 → 微盘回测收益系统性虚高。这是清单 A/B/G 里**唯一 material 且完全未量化**的执行 gap(封板/停牌对当前低换手 top-25 策略近乎不动针,见下「暂缓」条)。需:point-in-time 退市状态 + 退市整理期价格 → 在 `core.engine` 持仓清算逻辑里按真实清算价归零(非 drop)。谁:数据层(取退市状态/整理期价)+ 引擎侧(R-BT-001,改唯一权威须同步 SPEC + 补 test_engine)。
- [ ] **【引擎/口径·高优先】执行契约口径对齐:T+1 开盘 vs 收盘** — 已敲定 canonical 引擎成交在 **T+1 收盘**(`execution_timing="T_PLUS_1_CLOSE"`、`close.pct_change()`、`raw_open` 字段从未被读取);而 `回测执行现实核对清单.md` C 组及此前 gap 研究的 reality 层按 **T+1 开盘/VWAP**——差整整一天。**风险:前面所有执行 gap 的量级数(含下方 ΔSh)可能在引擎并不使用的口径上测出而失真**,实盘决策前必须先统一。需:定官方口径(收盘 or 开盘/VWAP);若改开盘须新增 `T_PLUS_1_OPEN` 分支用 raw_open + 立 ADR + 同步 SPEC。关联 backlog chip(raw_open 死脚手架清理)。谁:引擎侧 + 研究侧。
