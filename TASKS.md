...
- [ ] **前端债券轮动指引诚实化(2026-06-23 对话发现)** — `web/app/signals/page.tsx`(L158「🔵 债券轮动 (BEAR)」/L197「继续保持国债ETF避险持有状态」/L76「维持避险」)把买 511010 当**现行有效避险指引**展示,**全程无任何来源/时效标注**:该信号来自已被 ADR-020 降为「参考/不可部署」的 `illiquidity/v3.1`,且整条 paper/signal 自 06-18 起 fail-closed(`signals/2026-06-18.json` 是最后一条,`paper/account.json` 冻在 06-18,持有的 7000 股 511010 是 fail-closed 前买的遗留仓)。属"台账已判失效/页面仍 PASS"脱节类(同 STATUS 06-18 修过的类)。修法:债券轮动卡标注「来源策略已降级(参考)/信号 stale@2026-06-18/非现行可执行」,或在主腿未定前不展示为 actionable。小、可立刻做(纯展示层,不改研究口径,符 §14)。谁:Web 侧。
- [ ] **防御择时(MA16+国债轮动)与已降级主腿解耦(2026-06-23 对话发现)** — 用户质疑命中根因:买 511010 的**触发决策**是 `factors/small_cap.py::small_cap_timing` 的 MA16 择时(小盘指数跌破 16 日均线→bear→闲置资金全买国债),`ma=16` 是调出来的参数、当初搭在 illiquidity/v3.1 里一起回测,而 v3.1 **DSR 未过**(R-EVIDENCE-001 ④:择时/veto/网格全算搜索自由度,该计入 DSR 惩罚)。即:「国债是安全资产」成立,但「**何时切进国债**」这个核心决策**从未被独立验证**,且现在主腿一降级它就成孤儿(paper 引擎 `execute_to_target` 凭 `bond.enabled`=regime bear 下单,**不查 deployment defensive 腿**,与 run_daily 展示分支不是同一路径)。修法:把 MA16+国债轮动从 v3.1 剥成独立可部署、可独立验证的 defensive overlay(自带身份+自带证据轨,类比 diversifier 轨凭"少亏"准入但需明确 rationale),否则防御逻辑永远寄生在会死的 alpha 腿上。大、需设计+用户拍板优先级。谁:用户定向 + 研究/workflow。
...
- [ ] **既往 phase2/3 绩效用截断后引擎重算(ADR-021 后续)** — ADR-021 把 promote 验证栈(`phase2_backtest`/`phase3_wf`)截到 `<holdout boundary`,**既往在册策略的 phase2 三段(OOS 含 2025-26 金库)与 phase3 WF 数字都含金库、偏乐观**。需用截断后引擎重跑在册版本的 phase2/3,更新台账 OOS/WF 绩效与 data_scope。注:此为绩效诚实化,不改入册结论(入册仍以 hit+DSR+9-Gate 为准)。谁:走 workflow / 研究侧。
