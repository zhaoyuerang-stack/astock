...
- [ ] **生产部署清单指向一个从未真正过闸的策略** — `deployments/production.json` 唯一腿 `illiquidity/v3.1`,加载时报 `spec_hash 不匹配`(清单钉的哈希是修复前的)。但**根因更深**:`decide_nine_gate()`(部署严格闸,要求 `passed_all=True`)从 2026-06-19 到今天重审,从未对这条腿点头过——`migrate_deployment.py --apply` 会因此直接拒绝重新生成清单,不是改个哈希就能"修复"。是否换腿/降级/接受现状,需人决定。谁:用户。
...
- [ ] **三假货台账处置(ADR-017)** — ① `illiquidity-large-cap/v1.0` 退役/重分类(独立 IC=−0.084 反向、9-Gate 证据照抄小盘);② `industry-neglect-rotation/v1.3` 补独立 9-Gate(L2 复合 DSR/PBO)+ admission 标注「裸因子年化13.8%/回撤−60% 不达标,standalone 靠 MA16 overlay」;③ `ai-compute-toc/v1.0` logic chain `ai_compute_toc_bottleneck` 降级 + desc 失效标注(机制三重证否)。草稿 `scratch/{illiq_largecap_governance,toc_honest_conclusion}_DRAFT.md`。**处置后从 `scripts/ci/check_registry_evidence.py::PENDING_REMEDIATION` 移除对应 key**(守卫会提示)。谁:走 workflow。
- [ ] **CLAUDE.md 扩写(宪法,用户改)** — 铁律9 加「9-Gate 证据自证」5 条(ADR-017 C 段:禁跨家族照抄/config须可复现/门未实算禁 standalone/n_trials≥搜索规模/生成码在 canonical);文档矩阵顶部加 `LOOP_ENGINEERING.md` 指针。谁:用户(我不碰宪法)。
