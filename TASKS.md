...
- [x] **A0: 架构守卫补强(先防回流)** — ✅ 已把“API 薄层”和“workflow 不依赖 research script”从约定变成机械检查:`check_layer_deps.py` 现在会拦截新增 `workflow.* -> scripts.research.*` 依赖,以及新增 `api.*` 对 `data_lake/reports/signals/paper` 的直接读取;`services.read.*` 作为 artifact 读取边界不误报。当前仅为两处已知债务保留显式 allowlist:`workflow/promote.py`(A1 移除)与 `api/routers/experiments.py`(A2 移除)。验证:`cd factor_research && python3 tests/test_layer_deps_guard.py && python3 scripts/ci/check_layer_deps.py` 均通过。
- [x] **A1: 移除 `workflow/promote.py` → `scripts.research.run_nine_gates_all` 反向依赖** — ✅ 已抽 `factor_research/workflow/nine_gate_runner.py` 承载可复用 `run_evaluation/audit_stale_registered`, `workflow/promote.py::_default_nine_gate_runner` 改走 workflow 库入口,`scripts/research/run_nine_gates_all.py` 退成 CLI 包装;A0 中 `workflow/promote.py` 的显式 allowlist 已移除。验证:`cd factor_research && python3 tests/test_layer_deps_guard.py && python3 tests/test_promote_nine_gate.py && python3 scripts/ci/check_layer_deps.py && python3 scripts/research/run_nine_gates_all.py --help` 均通过。
- [x] **A2: 清空 `api/routers/experiments.py` 的直接文件读取** — ✅ 已将 `/experiments/shadow-incubation`、`/experiments/amount-timing-validation`、`/experiments/logical-chains`、`/experiments/industry-knowledge-graph` 的文件读取搬到 `services/read/experiments.py`;router 仅调用 service 函数;A0 中 `api/routers/experiments.py` 的 artifact 直读 allowlist 已移除。新增 `tests/test_experiments_read_services.py` 覆盖缺文件、坏 JSON fail-closed 与正常 JSON。验证:`cd factor_research && python3 tests/test_experiments_read_services.py && python3 tests/test_layer_deps_guard.py && python3 tests/test_api_contracts.py && python3 scripts/ci/check_layer_deps.py` 均通过。
- [x] **A3: 拆 `portfolio/strategy_runners.py` 的策略目录/runner/部署解析三重职责** — ✅ 已拆出 `portfolio/runner_protocol.py`、`portfolio/research_catalog.py`、`portfolio/deployment_runner.py`、`portfolio/runner_registry.py`;旧 `portfolio/strategy_runners.py` 退成兼容 re-export。`run_active()/active_strategies()/defensive_strategies()` 已由 deployment runner 承载,研究目录由 research catalog 承载,现有旧 import 仍可用。新增 `tests/test_portfolio_runner_boundaries.py` 覆盖 re-export、部署清单优先、research catalog fallback。验证:`cd factor_research && python3 -m pytest tests/test_portfolio_runner_boundaries.py tests/test_deployment_manifest.py -q && python3 scripts/ci/check_layer_deps.py && python3 -m py_compile portfolio/runner_protocol.py portfolio/research_catalog.py portfolio/deployment_runner.py portfolio/runner_registry.py portfolio/strategy_runners.py` 均通过。
- [x] **A4: 组合晋级逻辑从“脚本内重写策略公式”改为“引用版本规格/runner”** — ✅ 已新增 `workflow/composite_spec.py` 支持 `family/version:weight` 与旧 alias 显式映射、权重和校验;新增 `portfolio/composite_weight_runners.py` 承载 illiquidity/v3.1、hq-momentum-hedged/v1.0、reversal-composite/v1.0 的组合权重 runner;`portfolio.runner_registry.get_composite_weight_runner()` 负责解析 leg。`workflow/promote_composite.py` 已移除脚本内 AST/Amihud/大盘动量/dual-valve 公式块,只解析 allocation、调用 runner 生成 T0/T1 权重并执行 Gate 7B + 9-Gate。旧 alias 与显式 `family/version` 走同一 runner-backed 路径。验证:`cd factor_research && python3 tests/test_promote_composite_spec.py && python3 -m pytest tests/test_promote_composite_spec.py tests/test_portfolio_runner_boundaries.py tests/test_deployment_manifest.py -q && python3 -m py_compile workflow/promote_composite.py portfolio/composite_weight_runners.py portfolio/runner_registry.py && python3 scripts/ci/check_layer_deps.py` 均通过。
- [x] **A5: artifact/repository 层收敛路径常量** — ✅ 已新增 `runtime/artifacts.py::ArtifactPaths(root)` 作为运行产物路径唯一集中入口,只表达路径、不做 IO;已迁移 `services/read/{state,portfolio,paper,audit,trade_readiness,governance,experiments}.py` 中散落的 `data_lake/reports/paper/signals` 路径,并让 `research_ledger/workspace.py`、`factory/autoresearch/repositories.py` 的默认路径来自 `ArtifactPaths`。新增 `tests/test_artifact_paths.py` 覆盖默认项目根、`tmp_path` 注入与 repository 默认路径一致性。验证:`cd factor_research && python3 tests/test_artifact_paths.py && python3 tests/test_experiments_read_services.py && python3 tests/test_services_phase0.py && python3 -m pytest tests/test_services_phase0.py tests/test_research_workspace*.py tests/test_autoresearch_engine.py -q` 均通过。
- [x] **A6: `services` 接缝瘦身与权限分层** — ✅ 已在 `services/__init__.py` 明确三类端口:`read` 只读、`actions` 写/重任务必须经 `action_guard` 或 `jobs`、`agent` 只能编排 read/actions 工具;`check_layer_deps.py` 新增两条静态守卫:① `services.actions.*` 中触发 `workflow.promote`/`strategy_registry` 的高风险动作必须同模块显式依赖 `jobs` 或 `action_guard`;② API 层直接写运行审计产物必须显式引入 `services.actions.action_guard`。`services/actions/research_workspace.py` 的 L0-L3 stage runner 已下沉到 `workflow/research_stages.py`,service 保留旧 wrapper 与 job 协调。新增/扩展 `tests/test_layer_deps_guard.py`、`tests/test_research_workflow_actions.py`。验证:`cd factor_research && python3 tests/test_research_workflow_actions.py && python3 tests/test_layer_deps_guard.py && python3 tests/test_action_jobs_phase7.py && python3 scripts/ci/check_layer_deps.py && python3 -m pytest tests/test_research_workflow_actions.py tests/test_action_jobs_phase7.py -q` 均通过;完整 `bash scripts/test_all.sh` 待最终总验收运行。
- [x] **A7: 文档与架构图同步** — ✅ 已同步 `SPEC.md` 运行态图(加入 `workflow.nine_gate_runner`、`portfolio.runner_registry`、`runtime.artifacts`),`CLAUDE.md` §16 守卫表(API artifact 边界、services 权限分层、workflow/script 边界),`WORKFLOW.md` 的 9-Gate/组合晋级执行路径,并在 `DECISIONS.md` 追加 ADR-030 “服务接缝与组合 runner 解耦”。同时移除旧 `LIVE_STRATEGIES` 名称,旧 strategy_runners 入口只保留兼容 re-export。验收:`rg -n "scripts\\.research\\.run_nine_gates_all|LIVE_STRATEGIES|strategy_runners.py" CLAUDE.md SPEC.md WORKFLOW.md DECISIONS.md factor_research/portfolio factor_research/workflow` 无输出;`python3 -m pytest tests/test_portfolio_runner_boundaries.py tests/test_deployment_manifest.py -q`、`python3 tests/test_layer_deps_guard.py` 通过。

**推荐执行顺序**:A0 → A1 → A2 可作为第一批小提交;A3 → A4 是组合层较大重构,应单独分支/单独提交;A5 → A6 是服务层治理,不要和组合 runner 同时改;A7 最后随实际落地同步。每个 commit 必须显式 `git add <file>...`,提交前检查 `git diff --cached --stat && git diff --cached`。
