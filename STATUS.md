...
**2026-07-17(守卫绕过审计 + R-COST-001 hash-pin 机械化;Grok CLI 委托首单)**:
  ...
  · **#5 已修复(commit 6e8627e6..291e178b,Grok 委托第三单;owner 拍板=存量硬切+config-only 降级身份)**:新建 `lake/version_returns.py` canonical writer(此前根本不存在,直写 CSV 泛滥的根因)——写必填身份(spec_hash→tier=spec / config_hash→tier=config-only / 皆无 raise),sidecar 含 series_hash 绑数据防"换 CSV 留旧 sidecar"投毒,指纹复用 `governance.holdout.current_data_fingerprint`;`load_verified_version_returns` 校验读入口;recompose/paper 选择路径 fail-closed(无戳腿显式列 `excluded_no_provenance`,tier 透传 paper_candidates;监控路径不 gate,同 holdout 哲学);两处直写(run_nine_gates_all/promote_composite)迁 writer,lake 守卫 PENDING 销 2 条(余 8)、此后直写 version_returns 即硬红。存量无戳 CSV 自然被选择路径排除(在册池=0,硬切代价为零)。Claude 独立复验:投毒/冒名/无身份三类对抗全被拦 + 13 守卫绿 + 主仓全量 RC=0。合并时与他人在途 run_nine_gates_all 改动(窗口家族 runner 扩展)同文件不同段,stash→ff→pop 干净合回。
  · **审计终局**:9 项发现 7 项已机械强制(#1-#5/#7/#8),#6=文档补记待办,#9=固有局限不修。
  · **待办**:#6(env-var 人门仅桌面进程内成立)文档补记;PENDING 余项逐项销账(scratch force=True 遗留建议直接删);Grok 三问余两问(lake 误报升 AST / paper_forward 豁免口径 / factor_store 归属);TASKS 立项(他人在途编辑,仍未写入)。
