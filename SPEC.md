...
2. **策略工厂** ⏳ — 自进化引擎(`evolve`)批量发现候选 + 自动证伪(过拟合/幸存者偏差/特定行情);**按母策略隔离进化(岛屿模型,见下「防同质化」)**。当前仍旧口径+乐观成本,待真实化。
3. **有效策略管理** ✅台账 / ○监控 — 母策略两层台账,跟踪 有效/衰减/退役。
4. **中央调度层** ○ — event-driven 编排:数据就绪 / 市场状态切换 / 失效信号触发 → 启动或停用 对应母策略/组合;并负责定时/条件拉取数据。
5. **组合层** ○ — 从「有效」母策略组合(低相关加权 / 轮换,机制待定)。
6. **展示层** ○ — 向用户展示各策略/组合的收益与对比。

- 核心循环:工厂产出 → 管理层汰换 → **只有「有效」策略才进组合与展示**。
- ⚠ 依赖顺序:组合/展示的收益必须建在真实口径上;`evolve` 真实化(地基)未完成前,展示的收益含幸存者偏差水分,**不可示人**。
...
## 成本模型
见 CLAUDE.md「交易成本」表:往返 ≈0.47% + 融资 ~1.25%/年;佣金/融资用可谈硬费率,冲击滑点维持审慎不乐观化。
