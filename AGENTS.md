...
**价值判据(owner 2026-07-10 拍板,先于下面的分桶问题)**:

- 有助于找到能实盘的有效策略的功能与证据 → 可提交;
- 提升产品用户体验(Web / 小程序 / 桌面端)的功能 → 可提交;
- 任何导致自欺欺人的改动(为达标降成本、绕守卫、偷看 holdout、弱化断言、伪造/照抄证据) → **禁止提交**。
  自欺判定不靠自觉,交给机械守卫:**提交前 `factor_research/scripts/test_all.sh` 必须全绿**。
- **任务收尾必须提交**:一个意图完成即按本文纪律提交对应文件,不留过夜脏树。
  单人多工具开发下,"脏树"几乎总是"没收尾",不是"有并发";收尾提交是每个 session 的最后一步,不是可选项。
